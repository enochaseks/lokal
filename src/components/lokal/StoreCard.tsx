...
export function StoreCard({ store, onClick }: { store: Store; onClick: () => void }) {
  ...
  const isArtistService = ["Barbers", "Hair & Beauty", "Body Arts & Crafts"].includes(
    store.category,
  );
        ...
        <div className="flex flex-1 flex-col gap-2 p-5">
          ...
          <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-2 text-[10px] sm:text-[11px]">
            ...
            {store.is_open_now === false && (
              <span className="rounded-full bg-slate-200 px-2 py-0.5 font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                ⚫ Closed
              </span>
            )}
            ...
            {store.category === "Groceries" &&
              store.subcategory === "Meat & Fish" &&
              store.health_safety_certificate_status === "approved" && (
                <span className="rounded-full bg-emerald-100 px-2 py-0.5 font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                  ✅
                </span>
              )}
            ...
            {store.location_type === "travel" && (
              <span className="rounded-full bg-purple-100 px-2 py-0.5 font-medium text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">
                🚗
              </span>
            )}
            ...
            {!store.location_type && (
              <>
                ...
                {store.fulfillment === "pay_at_store" && (
                  <span className="rounded-full bg-yellow-100 px-2 py-0.5 font-medium text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300">
                    💰
                  </span>
                )}
              </>
          ...
          <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
            ...
            {store.reviews > 0 && (
              <span className="ml-2 flex shrink-0 items-center gap-0.5 font-medium text-amber-500">
                ...
                {store.rating} <span className="text-muted-foreground">({store.reviews})</span>
              </span>
            )}
          </div>
        </div>
          ...
          {previewUrl && (
            <div className="mt-2">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                If this site blocks embedded previews, use the merchant social or website links from
