  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tab === "signup") {
        ...
        toast.success("Check your email", { description: "We've sent a confirmation link." });
      } else {
    ...
    <div className="min-h-screen bg-gradient-hero flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-8 flex items-center justify-center gap-2">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-primary shadow-warm">
            ...
            <form onSubmit={handleEmailAuth} className="mt-5 space-y-4">
              <TabsContent value="signup" className="space-y-4 mt-0">
                <div>
                  ...
                  <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ama Boateng" maxLength={100} className="mt-1" />
                </div>
              </TabsContent>
              ...
              <Button type="submit" disabled={busy} className="w-full bg-gradient-primary text-primary-foreground shadow-warm hover:opacity-95" size="lg">
